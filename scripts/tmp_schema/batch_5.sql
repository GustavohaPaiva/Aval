-- >>> FILE: 20260727202950_produtos_oficiais_unique_by_quarter.sql
-- Allow same fertilizante name across different quarters (Cibra multi-quarter)
DROP INDEX IF EXISTS public.produtos_oficiais_fornecedor_nome_unique_idx;
DROP INDEX IF EXISTS public.produtos_oficiais_fornecedor_sku_unique_idx;

CREATE UNIQUE INDEX IF NOT EXISTS produtos_oficiais_fornecedor_nome_quarter_unique_idx
  ON public.produtos_oficiais (
    fornecedor_id,
    lower(TRIM(BOTH FROM nome)),
    lower(TRIM(BOTH FROM quarter))
  );
...
BEGIN
  ...
  LOOP
    ...
    -- Match by fornecedor + nome + quarter so Q3 and Q4 coexist
    ...
    WHERE po.fornecedor_id = v_lote.fornecedor_id
      ...
      AND lower(trim(po.quarter)) = lower(trim(COALESCE(NULLIF(trim(v_linha.quarter), ''), v_quarter)))
    LIMIT 1;
