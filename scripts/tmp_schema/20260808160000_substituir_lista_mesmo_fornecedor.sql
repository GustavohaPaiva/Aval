-- Ao lançar/reativar uma lista: no máximo 1 ativa por fornecedor+quarter+estado.
-- Listas anteriores são inativadas (histórico preservado); produtos órfãos da lista
-- antiga ficam inativos. Produtos da lista nova permanecem ativos.
...
BEGIN
  ...
  v_estado_padrao := NULLIF(trim(COALESCE(v_lote.estado_padrao, '')), '');
  IF v_estado_padrao IS NULL OR v_estado_padrao NOT IN ('MG', 'SP', 'RS') THEN
    RAISE EXCEPTION 'Estado padrão (MG/SP/RS) obrigatório antes de lançar a lista.';
  END IF;
  ...
  -- Inativa listas anteriores do mesmo fornecedor + quarter + estado
  SELECT coalesce(array_agg(l.id), ARRAY[]::uuid[])
  INTO v_old_ids
  FROM public.lotes_importacao l
  WHERE l.id <> p_lote_id
    AND l.fornecedor_id = v_lote.fornecedor_id
    AND l.status = 'concluido'::public.lote_importacao_status
    AND coalesce(l.ativo, true) = true
    AND lower(trim(coalesce(l.quarter_calculado, ''))) = lower(v_quarter)
    AND nullif(trim(coalesce(l.estado_padrao, '')), '') = v_estado_padrao;
  ...
  IF cardinality(v_old_ids) > 0 THEN
    ...
    GET DIAGNOSTICS v_listas_inativadas = ROW_COUNT;

    UPDATE public.produtos_oficiais
    SET ativo = false, updated_at = now()
    WHERE lote_id = ANY (v_old_ids);
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'listas_inativadas', v_listas_inativadas
  );
END;
$function$;
...
BEGIN
  ...
  IF length(v_quarter) > 0 AND v_estado_padrao IS NOT NULL THEN
    SELECT coalesce(array_agg(l.id), ARRAY[]::uuid[])
    INTO v_old_ids
    FROM public.lotes_importacao l
    WHERE l.id <> p_lote_id
...
COMMENT ON FUNCTION public.promover_lote_importacao(uuid) IS
  'Lança o lote: upsert produtos e inativa listas anteriores do mesmo fornecedor+quarter+estado.';

COMMENT ON FUNCTION public.reativar_lista_importacao(uuid) IS
  'Reativa a lista e inativa outras ativas do mesmo fornecedor+quarter+estado.';
